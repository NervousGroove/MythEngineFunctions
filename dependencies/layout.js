...
(function()
{
		...
		for (i = 0, len = active_effect_types.length; i < len; i++)
		{
			...
			if (i == 0 && !pre_draw)
			{
				...
				if (inst)
				{
					...
					// HACK for Sprite object: to make sure the pixel size takes in to account spritesheeting, use the current frame's
					// source image as the dimensions to determine the pixel size from.
	...
	Layer.prototype.rotateViewport = function (px, py, ctx)
	{
		...
		// If there is a negative scale, normalise the viewport. Otherwise everything fails the "inside viewport" test.
		var temp;
