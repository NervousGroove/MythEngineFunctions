...
(function()
{
	...
	function Runtime(canvas)
	{
		...
		// Identify WKWebView. This is Cordova on iOS (implying a webview), and uses the window.webkit property to distinguish
		// WKWebView from UIWebView. This is the same way Cordova itself distinguishes them.
	...
	Runtime.prototype.initRendererAndLoader = function ()
	{
		...
		// The WebGL powerPreference attribute requires that the context loss handling events are added *before*
		// the context is created. Therefore we add these now, even before we know which context will be retrieved.
		// It doesn't matter if we end up using canvas2d, the events will simply be unused.
		...
		if (this.gl)
		{
			...
			log("Using WebGL " + (isWebGL2 ? "2 " : "") + "renderer: " + this.glUnmaskedRenderer);
	...
	// C2 does not use any in-editor spritesheeting like C3 does, so can end up loading thousands of individual image files.
	// Chrome has a limit of ~6000 pending requests, and users have actually run in to using more images than this limit.
	// To work around this, use a queue of images and the src they need to be set on them, and only have 100 loading in parallel.
	// This approach is not needed for C3, since it uses spritesheeting even in preview mode, so even the very biggest projects
	// are reduced to a few hundred spritesheets - still comfortably within the limit.
	...
	Runtime.prototype.areAllTexturesAndSoundsLoaded = function ()
	{
		...
		for (i = 0, len = this.wait_for_textures.length; i < len; i++)
		{
			...
			// Note images with no src may be awaiting decompression from expansion APK, or simply
			// in the pending queue, so don't count them as completed
	...
	Runtime.prototype.setCtxImageSmoothingEnabled = function (ctx, e)
	{
		...
		{
			// Fall back to prefixes for compatibility. Only do this if standard property unavailable
			// to avoid console deprecation warnings.
