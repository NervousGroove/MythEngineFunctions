...
(function ()
{
	...
	instanceProto.tick2 = function ()
	{
		...
		// Position object to fill viewport exactly, so shadows can be rendered anywhere. Note we extend to fill a larger
		// area than the viewport (extended somewhat arbitrarily by max_extrude) since if an object is just outside the viewport,
		// it may still need to cast a shadow into the viewport.
	...
	instanceProto.getPolyPoints = function (poly, instx, insty, flipped)
	{
		...
		for (i = 0; i < poly_len; ++i)
		{
			...
			// note if object mirrored xor flipped, the points will come back in reverse order giving the wrong winding.
			// compensate by inserting in reverse order if this is the case.
			if (flipped)
			{
				poly_pts.unshift(y);
				poly_pts.unshift(x);
