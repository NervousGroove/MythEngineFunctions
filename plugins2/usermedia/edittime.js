...
AddCondition(13, cf_trigger, "On canvas recording ready", "Canvas recording", "On canvas recording ready", "Triggered after 'Stop recording canvas' when the recording is available in the CanvasRecordingURL expression.", "OnCanvasRecordingReady");

AddCondition(14, cf_none, "Is canvas recording supported", "Canvas recording", "Is canvas recording supported", "True if the current browser supports recording the canvas.", "SupportsCanvasRecording");

AddComboParamOption("WebM VP8");
AddComboParamOption("WebM VP9");
AddComboParam("Format", "The video format to test if supported for canvas recording.");
AddCondition(15, cf_none, "Is recording format supported", "Canvas recording", "Is canvas recording format <b>{0}</b> supported", "Check if a particular video format is supported for canvas recording.", "IsCanvasRecordFormatSupported");
...
AddNumberParam("Preferred width", "The preferred horizontal resolution. Leave at 0 for default.");
AddNumberParam("Preferred height", "The preferred vertical resolution. Leave at 0 for default.");
AddAction(0, af_none, "Request camera", "Media request", "Request camera source <i>{0}</i> (prefer {1} x {2})", "Prompt the user for access to video input from a camera, if one is present.", "RequestCamera");
...
AddComboParamOption("WebM VP8");
AddComboParamOption("WebM VP9");
AddComboParam("Format", "The video format to encode the canvas recording with. Note not all formats may be supported by all browsers/platforms.");
AddNumberParam("Quality (kbps)", "The data rate to encode in kilobits per second (kbps).", "2500");
AddNumberParam("Framerate", "The frames per second to capture in the video. Use 0 for the display rate.");
AddAction(11, af_none, "Start recording canvas", "Canvas recording", "Start recording canvas (<i>{0}</i> @ <i>{1}</i> kbps, <i>{2}</i> FPS)", "Record a real-time video of the canvas.", "StartRecordingCanvas");

AddAction(12, af_none, "Stop recording canvas", "Canvas recording", "Stop recording canvas", "Finish recording a canvas video. Triggers 'On canvas recording ready' when available.", "StopRecordingCanvas");
